...
function getProjectMonths(start: string | Date, end: string | Date): string[] {
  const s = new Date(start)
  const e = new Date(end)
...
// Each day contributes 1/5 of the value to whichever month it falls in.
function distributeWeekToMonths(weekStartDate: string, value: number): [string, number][] {
        ...
        <div className="w-full max-w-6xl max-h-[88vh] flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            {sub === 'Financial' && (() => {
              ...
              // Distribute staffing rows into months via working-day ratio
              ...
              // Other costs — equally split across months
              ...
              // Derive all computed fields per month
              ...
              // Project total — sum accumulatables, then re-derive rates
              const tot = zero()
              for (const r of byMonth.values()) {
              ...
              function fmtM(n: number) { return n !== 0 ? fmt(n) : <span className="text-slate-300">—</span> }
              function fmtRate(n: number) { return n > 0 ? `$${n.toFixed(2)}/hr` : <span className="text-slate-300">—</span> }
              function fmtHrs(n: number) { return n > 0 ? `${n.toFixed(1)} h` : <span className="text-slate-300">—</span> }
              function fmtPct(n: number, colored?: boolean) {
                const s = `${n.toFixed(1)}%`
                if (!colored) return s
              ...
              type RowDef = { id: string; label: string; get: (r: MRow) => ReactNode; bold?: boolean; indent?: boolean; separator?: boolean }
              const ROWS: RowDef[] = [
