                  ...
                  {!deleteConfirm ? (
                    ...
                    <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 space-y-3">
                      <p className="text-xs font-semibold text-red-700">
                        Opportunity once deleted cannot be reverted back. Are you sure?
                      </p>
                      <div className="flex gap-2">
                        <button
                          ...
                          onClick={async () => {
                            setDeleting(true)
                            try {
                              const res = await fetch(`/api/opportunities/${opp.opportunityId}`, { method: 'DELETE' })
                              if (!res.ok) { toast.error('Failed to delete opportunity'); setDeleting(false); return }
                        ...
                        <button
                          ...
                          className="rounded-md border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 transition-colors"
                        >
                          Cancel
                        </button>
                      </div>
          ...
          <div className="space-y-5">
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2 items-center">
                ...
                {!(oppStage === 'TO_BE_ARCHIVED' && oppStatus === 'OPEN') && (
